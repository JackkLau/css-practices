...
export const CarouselContainer = styled.div<{ sliding: boolean }>`
  ...
  transform: ${(props:any) => {
  if (!props.sliding) return "translateX(calc(-80% - 20px)) translateY(30px) translateZ(300px)";
  if (props.dir === PREV) return "translateX(calc(2 * (-80% - 20px))) translateY(30px)";
  return "translateX(0%) translateY(30px)";
}};
`;
